...
class CnbSCMFile : SCMFile {
    ...
    private constructor(
        ...
        apiContent = CnbContent("", "", "tree", 0)
        if (fallbackRepositoryPath != null) {
            fallbackContent = CnbContent("", "", "tree", 0)
        }
    }
    ...
    override fun newChild(
        ...
        CnbSCMFile(
            this,
            name,
            if (assumeIsDirectory) CnbContent(joinPath(path, name), "", "tree", 0) else null,
        )
    ...
    override fun content(): InputStream {
        ...
        val bytes =
            when (entry.encoding?.lowercase()) {
                null, "", "utf-8", "utf8", "text" -> {
                ...
                "base64" -> {
                    ...
                    throw IOException("Unsupported CNB content encoding '${entry.encoding}' for '$path'")
                }
            }
...
private fun CnbContent?.toFileType(): SCMFile.Type =
    when (this?.type?.lowercase()) {
        ...
        "file", "blob" -> SCMFile.Type.REGULAR_FILE
        "dir", "directory", "tree" -> SCMFile.Type.DIRECTORY
        "link", "symlink" -> SCMFile.Type.LINK
        else -> SCMFile.Type.OTHER
    }
