...
open class CnbSCMSource
    ...
    constructor(
        ...
        init {
            ...
            require(CnbRepositoryPath.isValid(this.repositoryPath)) {
                "CNB repository path must be canonical and contain a namespace and repository name"
            }
        }
        ...
        private fun readResolve(): Any {
            val restoredTraits: List<SCMSourceTrait>? = configuredTraits
        ...
        internal fun cloneUrlFor(path: String): String {
            require(CnbRepositoryPath.isValid(path)) { "CNB Git checkout repository path must be canonical" }
            val target = resolvedRepository
            if (target != null && target.path == path) {
                return secureCloneUrl(target)
            }
            return requireHttpsCloneUrl(derivedCloneUrl(path), path)
        }

        internal fun webUrlFor(path: String): String {
            require(CnbRepositoryPath.isValid(path)) { "CNB Web repository path must be canonical" }
            val target = resolvedRepository
            if (target != null && target.path == path) {
            ...
            return URI(
                ...
                configured.host,
                configured.port,
                "${configured.path.orEmpty().trimEnd('/')}/$path",
                null,
                null,
            ).toASCIIString()
        }
        ...
        override fun retrieve(
            ...
            client().use { client ->
                ...
                when (head) {
                    ...
                    is CnbTagSCMHead -> {
                        val tag = tagsFor(client, repository.path, setOf(head.name)).singleOrNull()
            ...
            client().use { client ->
                ...
                context.newRequest(this, listener).use { request ->
                    ...
                    if (request.fetchOriginPullRequests || request.fetchForkPullRequests) {
                        val pullRequests = pullRequestsFor(client, repository.path, request.requestedPullRequestNumbers)
                        request.pullRequests = pullRequests
                        if (request.fetchPullRequestLabels) {
                            request.pullRequestLabels = pullRequestLabelsFor(client, repository.path, pullRequests)
                        }
                        if (request.fetchPullRequestAuthorAccess) {
                            request.pullRequestAuthorAccess =
                                pullRequestAuthorAccessFor(client, repository.path, pullRequests)
                        }
                    }
                    if (request.fetchTags) {
                        request.tags = tagsFor(client, repository.path, request.requestedTagNames)
                    }
                ...
                when {
                    head is CnbPullRequestSCMHead && revision is CnbPullRequestSCMRevision -> {
                        ...
                        pullRequestCriteriaProbe(
                            ...
                            head,
                            revision,
                            isPullRequestTrusted(head, TaskListener.NULL, client),
                            ownsClient = true,
                        )
                    }
        ...
        private fun isPullRequestTrusted(
            ...
            CnbSCMSourceContext(null, SCMHeadObserver.none()).withTraits(traits).newRequest(this, listener).use { request ->
                if (!request.fetchPullRequestAuthorAccess) return request.isTrusted(head)
                val author = head.author.trim().takeIf(String::isNotEmpty) ?: return false

                fun evaluate(client: CnbClient): Boolean {
                    request.pullRequestAuthorAccess =
                        mapOf(author to pullRequestAuthorAccessFor(client, repositoryPath, author))
                    return request.isTrusted(head)
                }
                return suppliedClient?.let(::evaluate) ?: client().use(::evaluate)
            }
        }
        ...
        internal fun remember(repository: CnbRepository): CnbRepository {
            if (!repository.cloneable) {
                throw IOException(
                    "CNB repository '${repository.path}' is not cloneable in its current lifecycle or visibility state",
                )
            }
        ...
        internal fun pullRequestsFor(
            ...
            if (requested == null) {
                return client.listPullRequests(repository, CnbPullRequestListState.OPEN)
            }
            val pullRequests = ArrayList<CnbPullRequest>(requested.size)
            for (number in requested) {
                try {
                    val pullRequest = client.getPullRequest(repository, number)
                    if (pullRequest.state == CnbPullRequestState.OPEN) {
        ...
        internal fun tagsFor(
            ...
            if (requested == null) return client.listTags(repository)
            val tags = ArrayList<CnbTag>(requested.size)
            for (name in requested) {
                try {
                    tags += client.getTag(repository, name)
                } catch (failure: CnbApiException) {
        ...
        internal fun pullRequestLabelsFor(
            client: CnbClient,
            repository: String,
            pullRequests: Iterable<CnbPullRequest>,
        ): Map<String, Set<String>> {
            val labels = linkedMapOf<String, Set<String>>()
            for (pullRequest in pullRequests) {
                labels[pullRequest.number] =
                    try {
                        val names = linkedSetOf<String>()
                        for (label in client.listPullLabels(repository, pullRequest.number)) names += label.name
                        names
                    } catch (failure: CnbApiException) {
        ...
        internal fun pullRequestAuthorAccessFor(
            client: CnbClient,
            repository: String,
            pullRequests: Iterable<CnbPullRequest>,
        ): Map<String, Set<String>> {
            val accessByAuthor = linkedMapOf<String, Set<String>>()
            for (pullRequest in pullRequests) {
                val author = pullRequest.author
                if (author.isNotBlank() && author !in accessByAuthor) {
                    accessByAuthor[author] = pullRequestAuthorAccessFor(client, repository, author)
                }
            }
            return accessByAuthor
        }

        private fun pullRequestAuthorAccessFor(
            client: CnbClient,
            repository: String,
            author: String,
        ): Set<String> =
            try {
                val accessLevels = linkedSetOf<String>()
                for (membership in client.listMemberAccessLevels(repository, author)) {
                    accessLevels += membership.accessLevel.wireValue
                }
                accessLevels
            } catch (failure: CnbApiException) {
                if (failure.statusCode !in missingOrUnauthorizedStatusCodes) throw failure
