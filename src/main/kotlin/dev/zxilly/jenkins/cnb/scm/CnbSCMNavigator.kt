...
open class CnbSCMNavigator
    ...
    constructor(
        ...
        init {
            ...
            require(
                this.namespace.isNotBlank() &&
                    this.namespace.split('/').none { it.isBlank() || it == "." || it == ".." },
            ) {
                "CNB namespace must contain at least one non-relative path segment"
            }
        }
        ...
        override fun id(): String = "$serverId::$namespace"

        override fun visitSources(observer: SCMSourceObserver) {
            newRequest(observer).use { request ->
                client(observer.context).use { client ->
                    val repositories = client.listRepositories(namespace, request.includeDescendants)
        ...
        class DescriptorImpl : SCMNavigatorDescriptor() {
            ...
            fun doCheckNamespace(
                ...
                checkConfigurePermission(context)
                return if (value.isNullOrBlank() || !value.trim().matches(Regex("[^/\\s]+(?:/[^/\\s]+)*"))) {
