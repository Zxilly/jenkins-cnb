...
class CnbSCMSourceContext(
    ...
    var wantsForkPullRequests: Boolean = false
        private set
    var wantsPullRequestLabels: Boolean = false
        private set
    var wantsPullRequestAuthorAccess: Boolean = false
        private set
    internal var pullRequestCommentPolicy: CnbPullRequestCommentPolicy? = null
        private set
    ...
    fun wantPullRequestLabels(include: Boolean): CnbSCMSourceContext =
        apply {
            wantsPullRequestLabels = wantsPullRequestLabels || include
        }

    fun wantPullRequestAuthorAccess(include: Boolean): CnbSCMSourceContext =
        apply {
            wantsPullRequestAuthorAccess = wantsPullRequestAuthorAccess || include
        }

    internal fun withPullRequestCommentPolicy(policy: CnbPullRequestCommentPolicy): CnbSCMSourceContext =
        apply {
            pullRequestCommentPolicy = policy
        }
