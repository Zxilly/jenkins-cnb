...
internal object CnbBuildMetadataReporter {
    ...
    internal fun reportWithClient(
        ...
        if (mode.reportsAnnotations() && client.capabilities.supportsCommitAnnotations) {
            try {
    ...
    private fun updateAnnotations(
        ...
        client.putCommitAnnotations(target.commitRepository, target.sha, annotations(prefix, snapshot))
    }
    ...
    private fun annotations(
        ...
        listOf(
            CnbCommitAnnotation("${prefix}state", snapshot.state.wireName),
            CnbCommitAnnotation("${prefix}url", snapshot.buildUrl.take(2_000)),
            CnbCommitAnnotation("${prefix}build", snapshot.buildDisplayName.take(500)),
            CnbCommitAnnotation("${prefix}context", snapshot.target.context.take(500)),
            CnbCommitAnnotation("${prefix}updated_at", snapshot.stateChangedAt),
            CnbCommitAnnotation("${prefix}kind", "jenkins_build_metadata"),
        )
