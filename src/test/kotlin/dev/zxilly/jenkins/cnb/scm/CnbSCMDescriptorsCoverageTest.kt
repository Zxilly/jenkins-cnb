...
class CnbSCMDescriptorsCoverageTest {
    ...
    fun `discovery trait descriptors publish strategies categories and trust policies`(jenkins: JenkinsRule) {
        ...
        val branchProperties = CnbBranchPropertyFilterTrait.DescriptorImpl()
        val pullRequestFilter = CnbPullRequestFilterTrait.DescriptorImpl()
        ...
        assertEquals(listOf("0", "1", "2"), branchProperties.doFillProtectedStrategyIdItems().map { it.value })
        assertEquals(listOf("0", "1", "2"), branchProperties.doFillLockedStrategyIdItems().map { it.value })
        assertEquals(CnbSCMSourceContext::class.java, pullRequestFilter.contextClass)
        ...
        assertTrue(fork.getTrustDescriptors().any { it is TrustMembers.DescriptorImpl })
    }
    ...
    fun `authority descriptors constrain origins and expose production labels`(jenkins: JenkinsRule) {
        ...
        val members = TrustMembers.DescriptorImpl()
        ...
        assertEquals("Target repository members", members.displayName)
        assertEquals(listOf("Reporter", "Developer", "Master", "Owner"), members.doFillMinimumRoleItems().map { it.value })
        ...
        assertTrue(members.isApplicableToOrigin(SCMHeadOrigin.Fork::class.java))
    }
    ...
    fun `strategy objects retain exact checkout selections`(jenkins: JenkinsRule) {
        ...
        assertThrows(IllegalArgumentException::class.java) { CnbBranchPropertyFilterTrait(-1, 0) }
        assertThrows(IllegalArgumentException::class.java) {
            CnbPullRequestFilterTrait(false, "**", "**", "ci", "ci")
        }
    }
}
