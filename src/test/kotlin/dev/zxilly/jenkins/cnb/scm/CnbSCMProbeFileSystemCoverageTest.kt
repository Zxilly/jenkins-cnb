...
class CnbSCMProbeFileSystemCoverageTest {
    ...
    fun `owned fallback probe exposes metadata and closes its client exactly once`() {
                ...
                when (method) {
                    "getContent" -> {
                        ...
                        if (repository == "target/repo" && path == "Jenkinsfile") {
                            CnbContent(path, "blob", "file", 4, "echo", "text")
                        } else {
                            null
                        }
                    }
        ...
        val revision = CnbBranchSCMRevision(CnbBranchSCMHead("main"), "base")
        val probe =
            ...
            client { method, arguments ->
                when (method) {
                    "getContent" -> {
                        ...
                        CnbContent("dir/file", "link", "symlink", 0, "target", "utf8")
                    }
    ...
    fun `SCM file overlays directory entries and prefers primary file content`() {
        val primaryRoot =
            CnbContent(
                ...
                entries =
                    listOf(
                        CnbContentEntry("", "primary.txt", "p", "blob", 7),
                        CnbContentEntry("shared.txt", "shared.txt", "p2", "file", 7),
                    ),
            )
        val fallbackRoot =
            CnbContent(
                ...
                entries =
                    listOf(
                        CnbContentEntry("fallback.txt", "fallback.txt", "f", "file", 8),
                        CnbContentEntry("shared.txt", "shared.txt", "f2", "file", 8),
                    ),
            )
        val client =
            client { method, arguments ->
                ...
                when (repository to path) {
                    ...
                    "primary/repo" to "shared.txt" -> CnbContent(path, "p2", "file", 7, "primary", "text")
                    "fallback/repo" to "fallback.txt" -> CnbContent(path, "f", "file", 8, "fallback", "utf-8")
                    else -> null
                }
            }
    ...
    fun `SCM file reports missing and malformed content precisely`() {
        val values =
            mapOf(
                "plain" to CnbContent("plain", "1", "blob", 4, "text", null),
                "base64" to
                    CnbContent(
                        "base64",
                        "2",
                ...
                "bad64" to CnbContent("bad64", "3", "file", 1, "%%%", "base64"),
                "binary" to CnbContent("binary", "4", "file", 1, "x", "binary"),
                "empty" to CnbContent("empty", "5", "file", 0, null, "text"),
                "dir" to CnbContent("dir", "6", "dir", 0),
                "other" to CnbContent("other", "7", "commit", 0),
            )
        val client =
            client { method, arguments ->
        ...
        assertThrows(IOException::class.java) { root.child("binary").content() }
    ...
    fun `file system builder resolves branch and tag revisions and enforces lifecycle`() {
        ...
        val client =
            repositoryClient(
                onCall = { method, arguments ->
                    when (method) {
                        "getBranch" -> {
                            CnbBranch(arguments[1] as String, "branch-sha")
                        }

                        "listTags" -> {
                            listOf(CnbTag("v1", "tag-sha", 456))
                        }

                        "getContent" -> {
                            CnbContent("", "tree", "tree", 0)
                        }
        ...
        assertEquals("branch-sha", (branch!!.revision as CnbBranchSCMRevision).hash)
        ...
        val tagClient =
            repositoryClient(onCall = { method, _ ->
                when (method) {
                    "listTags" -> listOf(CnbTag("v1", "tag-sha", 456))
                    "close" -> Unit
    ...
    fun `file system builder rejects mismatched and unavailable revisions and closes clients`() {
        ...
        assertThrows(IOException::class.java) {
            builder.build(source, head, CnbPullRequestSCMRevision(otherHead, "base", "head", null))
        }
        assertEquals(1, closeCount)
