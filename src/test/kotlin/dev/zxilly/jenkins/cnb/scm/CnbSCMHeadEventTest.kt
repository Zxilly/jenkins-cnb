...
class CnbSCMHeadEventTest {
    ...
    fun `navigator project names are stable across namespace depth`() {
        ...
        val allRepositories = CnbSCMNavigator("cnb-cool", "").also { it.setDiscoverAllRepositories(true) }
        ...
        assertTrue(event.isMatch(allRepositories))
        assertFalse(event.isMatch(CnbSCMNavigator("other", "").also { it.setDiscoverAllRepositories(true) }))
        assertNotEquals(
    ...
    @Test
    fun `commit additions and all pull request events map to SCM lifecycle types`() {
        assertEquals(
            SCMEvent.Type.UPDATED,
            CnbSCMHeadEvent.typeOf(payload(CnbWebhookEvent.COMMIT_ADD, before = OLD_SHA, current = CURRENT_SHA)),
        )
        val opened = pullRequest(sourceRepository = "acme/repo", action = "opened")
        for (
        event in
        listOf(
            ...
            assertEquals(SCMEvent.Type.CREATED, CnbSCMHeadEvent.typeOf(payload(event, pullRequest = opened)))
            assertEquals(
                SCMEvent.Type.UPDATED,
                CnbSCMHeadEvent.typeOf(payload(event, pullRequest = opened.copy(action = "synchronize"))),
            )
        }
        for (
        event in
        listOf(
            ...
            assertEquals(SCMEvent.Type.UPDATED, CnbSCMHeadEvent.typeOf(payload(event, pullRequest = opened)))
        }
        assertEquals(
            SCMEvent.Type.REMOVED,
            CnbSCMHeadEvent.typeOf(payload(CnbWebhookEvent.PULL_REQUEST_MERGED, pullRequest = opened)),
        )
    }
    ...
    @Test
    @WithJenkins
    fun `event fetch applies authoritative branch property filters`(jenkins: JenkinsRule) {
        assertTrue(jenkins.jenkins.isUseSecurity.not())
        val client =
            object : CnbClient by targetedNavigatorClient(mutableListOf()) {
                override fun getBranch(
                    repo: String,
                    name: String,
                ): CnbBranch = CnbBranch(name, CURRENT_SHA, protected = false, locked = false)
            }
        val source = EventTestSource(client)
        source.setTraits(
            listOf(
                CnbBranchDiscoveryTrait(3),
                CnbBranchPropertyFilterTrait(protectedStrategyId = 1, lockedStrategyId = 0),
            ),
        )
        val event =
            CnbSCMHeadEvent(
                delivery(payload(CnbWebhookEvent.PUSH, refName = "release", current = CURRENT_SHA)),
            )
        val collector = SCMHeadObserver.collect()

        source.fetch(collector, event, TaskListener.NULL)

        assertTrue(event.heads(source).isNotEmpty(), "The signed event advertises the changed ref")
        assertTrue(collector.result().isEmpty(), "The authoritative unprotected API branch must be filtered")
    }

    @Test
    @WithJenkins
    fun `event fetch applies authoritative pull request filters`(jenkins: JenkinsRule) {
        assertTrue(jenkins.jenkins.isUseSecurity.not())
        val authoritative =
            CnbPullRequest(
                number = "42",
        ...
        val client =
            object : CnbClient by targetedNavigatorClient(mutableListOf()) {
                override fun getPullRequest(
                    repo: String,
                    number: String,
                ): CnbPullRequest = authoritative

                override fun getBranch(
                    repo: String,
                    name: String,
                ): CnbBranch = CnbBranch(name, TARGET_SHA)
            }
        val source = EventTestSource(client)
        source.setTraits(
            listOf(
            ...
            CnbSCMHeadEvent(
                delivery(
                    payload(
                        CnbWebhookEvent.PULL_REQUEST_UPDATE,
                        refName = "feature",
                        pullRequest = pullRequest(sourceRepository = "acme/repo", action = "synchronize"),
                    ),
                ),
            )
        val collector = SCMHeadObserver.collect()
        ...
        assertTrue(event.heads(source).isNotEmpty(), "The signed event advertises the changed PR")
        assertTrue(collector.result().isEmpty(), "The authoritative API draft flag must be filtered")
    }
