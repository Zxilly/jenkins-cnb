...
class CnbSCMNavigatorCoverageTest {
    ...
    fun `navigator descriptor and metadata actions expose configured namespace`(jenkins: JenkinsRule) {
        val server = CnbServer("primary", "Primary", "https://cnb.cool/root", "https://api.cnb.cool")
        ...
        assertEquals("https://cnb.cool/root/team", action.objectUrl)
        ...
        assertEquals(FormValidation.Kind.OK, descriptor.doCheckNamespace(null, "team/sub").kind)
        assertEquals(FormValidation.Kind.ERROR, descriptor.doCheckNamespace(null, "team//sub").kind)
