...
class CnbClientFactoryTest {
    ...
    fun `rejects missing and unsupported credential kinds`(jenkins: JenkinsRule) {
        ...
        assertEquals("The configured CNB credential was not found in this item context", missing.message)
        assertEquals("The configured CNB credential must be secret text or username/password", unsupported.message)
        assertTrue(
            missing.message
                .orEmpty()
                .contains("missing")
    ...
    fun `creates clients with explicit server-default and anonymous credential selection`(jenkins: JenkinsRule) {
        ...
        assertEquals("The configured CNB credential was not found in this item context", missingDefault.message)
