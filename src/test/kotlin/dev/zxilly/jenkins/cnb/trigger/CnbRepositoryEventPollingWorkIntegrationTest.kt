...
class CnbRepositoryEventPollingWorkIntegrationTest {
    ...
    fun `production polling recovers a classic job and persists cursor and dedup state`(jenkins: JenkinsRule) {
        ...
        try {
            ...
            val work = CnbRepositoryEventPollingWork()
            ...
            assertEquals(
                completedHour,
                CnbRepositoryEventCursorStore(cursorPath).get("primary", "team/project", completedHour),
            )
            val key =
            ...
            assertTrue(CnbRepositoryEventDedupStore(dedupPath).contains("primary", "team/project", key, Instant.now()))
        } finally {
            api.stop(0)
        }
    }
    ...
    fun `one repository API failure is logged without blocking a healthy repository`(jenkins: JenkinsRule) {
            ...
            startApi { exchange ->
                when {
                    exchange.requestURI.path.startsWith("/events/team/broken/") -> {
                        respond(exchange, 400, "{\"errcode\":\"INVALID_ARGUMENT\",\"errmsg\":\"broken archive\"}")
                    }
        ...
        try {
            ...
            CnbRepositoryEventPollingWork().runOnce(listener)
            jenkins.waitUntilNoActivity()
    ...
    @Test
    fun `SCM source discovery falls back from a missing item credential and becomes idle until due`(jenkins: JenkinsRule) {
        ...
        try {
            ...
            assertEquals(2, requestsAfterFirstPoll)
