...
- `api`：有界、强类型、与 Jenkins Job 行为无关的 CNB OpenAPI 客户端；
...
Jackson Core 仅在 typed decode 前作为流式词法守卫，用来拒绝重复 key、非法 UTF-8、过深嵌套、
过长字符串/数字和过多 token；`JsonNode`、`ObjectMapper`、反射对象绑定以及
...
annotation 使用 context 派生的 `jenkins/...` key。CNB 按 key 合并 PUT，插件只写自己的命名空间，
