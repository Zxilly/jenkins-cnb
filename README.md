...
| **Automatically update build status badge** | 自动把 queued、running 和最终 Jenkins 状态上传到 CNB Badge。升级现有安装时默认关闭。 |
| **Build status badge key** | 自动状态使用的 Badge key；默认 `security/tca`，最终允许值由 CNB 服务端决定。 |
...
在 CNB Server 中启用 **Automatically update build status badge** 后，插件会使用现有 Queue/Run listener 和
持久化重试队列自动上传 `queued`、`running`、`success`、`unstable`、`failure`、`aborted` 或
`not_built`。不需要在 Jenkinsfile 中调用 `cnbUploadBadge`。

自动 Badge 使用构建 Commit SHA、Jenkins Build URL 和配置的 Badge key，并更新 `latest` URL。Fork Pull
Request 会写入实际拥有该 Commit SHA 的源仓库。Reporting credential 需要 `repo-commit-status:rw`；Badge
失败进入异步重试，但不会修改 Jenkins Build Result。

`cnbSkipReporting` trait 同时关闭该 Branch Source 的自动 Badge、评论和 annotation。显式 Pipeline step
不受影响。

### 自定义 Badge 示例

`cnbUploadBadge` 保留用于非构建状态类的自定义指标。在 checkout 后调用上传步骤，确保 `env.GIT_COMMIT`
包含完整 Commit SHA。
...
Multibranch 构建可以自动上报 queued、running 和最终状态。启用 Server 的自动 Build Badge 后，同一持久化
生命周期也会自动更新 CNB Badge。Classic Job 使用
...
自动 Badge 独立于 **Build result reporting** 的评论/annotation 模式；即使该模式关闭，只要自动 Badge 已启用，
Badge 仍会更新。
