...
- 使用 `kotlinx.serialization` DTO 校验 CNB OpenAPI 与 Webhook 的 JSON 请求/响应边界；
...
2. PR 评论、评审、标签等只读查询：按所用步骤增加 `repo-notes:r`；启用成员信任或 PR 评论触发
   时增加 `repo-manage:r`，用于实时查询目标仓库成员权限；
...
TLS 要求；界面中的不安全 HTTP 开关只用于完全隔离的本地测试，不能用于生产。Payload 中携带的
